...
powershell -NoProfile -Command "$ready = $false; 1..40 | ForEach-Object { try { if ((Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:3000/compare.html' -TimeoutSec 1).StatusCode -eq 200) { $ready = $true; break } } catch {}; Start-Sleep -Milliseconds 250 }; if (-not $ready) { exit 1 }"
if errorlevel 1 (
  echo 엔진을 시작하지 못했습니다.
